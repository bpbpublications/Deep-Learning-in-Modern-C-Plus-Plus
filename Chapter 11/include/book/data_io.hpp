...
std::tuple<Tensor_2D, Tensor_2D, Tensor_2D, Tensor_2D> load_mnist(const std::string &folder, RANDOM_GEN &random_generator) {
    ...
    std::shuffle(test_indexes.begin(), test_indexes.end(), random_generator);
...
template <typename RANDOM_GEN>
Tensor_2D load_mnist_chunck(const std::string &folder, long int size, RANDOM_GEN &random_generator) {
    ...
    std::shuffle(indexes.begin(), indexes.end(), random_generator);
