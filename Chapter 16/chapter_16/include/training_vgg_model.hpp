...
protected:
    ...
    // subtracting ImageNet per-channel means
    ...
    void fill_batch(int begin, int end, Batch<4,2> &batch) {
        ...
        float scale = 1.f; // ImageNet does not scale
...
public:
    ...
    virtual void processing_image(cv::Mat &src, cv::Mat &dest) {
        ...
        rotated = rotate_and_translate(flipped, angle, horizontal_delta, vertical_delta);
...
void visualize(const Batch<4,2> * batch) {
    ...
    while(key != 27 && key >= 0)
    {
        ...
        Eigen::Tensor<float, 3> image_tensor_255 = image_tensor; // VGG does not scale
        ...
        std::cout << "instance: " << instance << "\tLabel index: " << label << "\n";
        cv::Mat dest;
