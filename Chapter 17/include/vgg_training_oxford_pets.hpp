...
const cv::Scalar IMAGENET_MEAN = cv::Scalar(103.939f, 116.779f, 123.68f);

const int NUMBER_CLASSES = CLASSES.size();
...
struct HybridBatch {
    ...
    HybridBatch(const Dim_Array<4> x_dims, const Dim_Array<2> t_dims, const Dim_Array<2> b_dims) {
        this->X = Tensor<4>(x_dims);
        this->T = Tensor<2>(t_dims);
        this->B = Tensor<2>(b_dims);
    }

    HybridBatch() : HybridBatch(Dim_Array<4>({0}), Dim_Array<2>({0}), Dim_Array<2>({0})) {}
...
public:
    ParallelBatches(Generator& gen, int batch_size, const Eigen::Tensor<std::string, 2> *data, int buffer_size, int image_size, bool center_images): 
        ...
        this->buffer = std::vector<HybridBatch>(this->buffer_size, HybridBatch());
    ...
    */
    void release(HybridBatch* batch) {
...
protected:
    ...
    virtual void processing_image(cv::Mat &src, cv::Mat &dest, int &xmin, int &ymin, int &xmax, int &ymax) {
        ...
        src.convertTo(converted, CV_32F);

        dest = converted - IMAGENET_MEAN;
    }

    void fill_batch(int begin, int end, HybridBatch &batch) {

        const int batch_size = end - begin;

        batch.X = Tensor_4D(batch_size, this->image_size, this->image_size, 3);
        batch.T = Tensor_2D(batch_size, NUMBER_CLASSES);
        ...
        for (int i = 0; i < batch_size; ++i) {
            ...
            if (clazz.compare("dog") == 0) {
                batch.T(i, 1) = TYPE(1.);
            } else {
                batch.T(i, 0) = TYPE(1.); // cat
            }
...
private:
    ...
    void load_batch_helper() {
        ...
        HybridBatch & batch = this->buffer[this->write_pointer];
...
public:
    ...
    virtual void processing_image(cv::Mat &src, cv::Mat &dest, int &xmin, int &ymin, int &xmax, int &ymax) {
        ...
        cv::Mat flipped, zoomed, rotated, converted;
        ...
        rotated = rotate_and_translate(zoomed, angle, horizontal_delta, vertical_delta);

        rotated.convertTo(converted, CV_32F);
...
void visualize(const HybridBatch * batch) {
    ...
    while(key != 27 && key >= 0)
    {
        ...
        Eigen::Tensor<float, 3> image_tensor_255 = image_tensor;

        Eigen::Tensor<float, 1> label_tensor = batch->T.chip<0>(instance);
        ...
        std::cout << "instance: " << instance << "\tLabel: " << CLASSES[label] << "\n";
        cv::Mat dest;
...
void check(const HybridBatch * batch, MODEL &model) {
    ...
    while(key != 27 && key >= 0)
    {
        ...
        Tensor_3D image_tensor_255 = image_tensor;
        ...
        auto [clazz, bbox_pred] = model.predict(input);
        
        Tensor_1D bbox_ground_truth = batch->B.chip<0>(instance);
        
        cv::Mat image;
        cv::eigen2cv(image_tensor_255, image);
        Eigen::Index label = ((Eigen::Tensor<Eigen::Index, 0>)clazz.argmax())(0);
        ...
        BoundingBox TRUE;
        TRUE.xmin = bbox_ground_truth(0);
        TRUE.ymin = bbox_ground_truth(1);
        TRUE.xmax = bbox_ground_truth(2);
        TRUE.ymax = bbox_ground_truth(3);

        BoundingBox PRED;
        PRED.xmin = bbox_pred(0, 0);
        PRED.ymin = bbox_pred(0, 1);
        PRED.xmax = bbox_pred(0, 2);
        PRED.ymax = bbox_pred(0, 3);

        float IoU = intersection_over_union(TRUE, PRED);

        cv::Mat output_image(cv::Size(dest.rows, dest.cols + 30), CV_8UC3, cv::Scalar(255, 255, 255));
        dest.copyTo(output_image(cv::Rect(0, 0, dest.rows, dest.cols)));

        std::stringstream stream;
        stream << CLASSES[label] << " - IoU: " << std::fixed << std::setprecision(2) << IoU;
        std::string msg = stream.str();
        cv::putText(output_image, msg, cv::Point(5, dest.rows + 21), cv::FONT_HERSHEY_DUPLEX, 0.8, cv::Scalar(0, 0, 0), 2);
...
void training(MODEL &model, GEN &rng, const Eigen::Tensor<std::string, 2> &training_data, 
    ...
    while (epoch < max_epochs) {
        ...
        HybridBatch* batch = batches.next();

        int training_success = 0;
        TYPE classification_training_loss = 0;
        ...
        while (batch) {
            ...
            if (steps > 0) {
                ...
                temp_update_desc = temp_update_desc + " classif train loss " + std::to_string(classification_training_loss / steps);
            ...
            model.backward(batch->T, batch->B);
            model.update(learning_rate, epoch + 1);

            // computing batch metrics
            auto classification_pred = model.get_class_prediction();
            training_success += count_success(batch->T, classification_pred);
            classification_training_loss += classifier_cost_fn.evaluate(batch->T, classification_pred);
        ...
        classification_training_loss /= steps;
        ...
        HybridBatch* val_batch = validation_batches.next();

        int validation_success = 0;
        TYPE classification_validation_loss = 0;
        TYPE regression_validation_loss = 0;
        int val_steps = 0;
        while (val_batch) {
            
            auto [classification, regression] = model.predict(val_batch->X);
            validation_success += count_success(val_batch->T, classification);
            classification_validation_loss += classifier_cost_fn.evaluate(val_batch->T, classification);
        ...
        classification_validation_loss /= val_steps;
        regression_validation_loss /= val_steps;

        std::cout 
                << "epoch:\t" << epoch << "\t"
                << "took:\t" << format_time(duration.count()) << "\t"
                << "\tcla train loss:\t" << classification_training_loss
                << "\treg train loss:\t" << regression_training_loss
                << "\tcla train acc:\t" << training_acc
                << "\tcla val loss:\t" << classification_validation_loss
                << "\treg val loss:\t" << regression_validation_loss
    ...
    ParallelBatches test_batches(rng, minibatch_size, &validation_data, batch_buffer_size, IMG_SIZE, center_images);
    test_batches.start();
    HybridBatch* test_batch = test_batches.next();

    TYPE IoU_ACC = TYPE(0.);
    while (test_batch) {

        auto [_, predictions] = model.predict(test_batch->X);
        TYPE batch_IoU = TYPE(0.);

        for (int i = 0; i < predictions.dimension(0); ++i) {

            BoundingBox TRUE;
            TRUE.xmin = test_batch->B(i, 0);
            TRUE.ymin = test_batch->B(i, 1);
            TRUE.xmax = test_batch->B(i, 2);
            TRUE.ymax = test_batch->B(i, 3);

            BoundingBox PRED;
            PRED.xmin = predictions(i, 0);
            PRED.ymin = predictions(i, 1);
            PRED.xmax = predictions(i, 2);
            PRED.ymax = predictions(i, 3);

            batch_IoU += intersection_over_union(TRUE, PRED);

        }

        std::cout << "batch avg IoU: " << (batch_IoU / predictions.dimension(0)) << "\n";
    ...
    TYPE IoU_avg = IoU_ACC / validation_data.dimension(0);

    std::cout << "IoU_avg: " << IoU_avg << "\n";
